...
module.exports.News_All = async (req, res) => {
  try {
    await News.find({ "infor.state_news": 1 })
...
module.exports.GetNameCity = async (req, res) => {
  try {
    let code_city = req.params.code_city;
    // if (code_city && code_city < 10) {
    //   code_city = "0" + code_city;
    // } else {
      
    // }
    // code_city = code_city.toString();

    // console.log("code_city 49: ", req.params.code_city);

    // const city = await Citys.findOne({ code: code_city }).exec();
...
module.exports.GetNameDictrict = async (req, res) => {
  try {
    let code_dictrict = req.params.code_dictrict;
    // if (code_dictrict && code_dictrict < 10) {
    //   code_dictrict = "00" + code_dictrict;
    // } else if (code_dictrict && code_dictrict > 9 && code_dictrict < 100) {
    //   code_dictrict = "0" + code_dictrict;
    // }
    // code_dictrict = code_dictrict.toString();

    // console.log("code_dictrict:", code_dictrict);

    await Districts.findOne({ code: code_dictrict }, (err, result) => {
      // console.log("result: ", result);
      if (err) {
        console.log(err);
      } else {
...
module.exports.News_RoomHome = async (req, res) => {
  try {
    await News.find({ "infor.state_news": 1, "infor.typehome": 1 })
      .sort({ "infor.date_now": -1 })
      .limit(6)
      .exec((err, result) => {
        if (err) console.log(err);
...
module.exports.News_HouseHome = async (req, res) => {
  try {
    await News.find({ "infor.state_news": 1, "infor.typehome": 2 })
      .sort({ "infor.date_now": -1 })
      .limit(6)
      .exec((err, result) => {
        if (err) console.log(err);
...
module.exports.News_ApartmentHome = async (req, res) => {
  try {
    await News.find({ "infor.state_news": 1, "infor.typehome": 3 })
      .sort({ "infor.date_now": -1 })
      .limit(6)
      .exec((err, result) => {
        if (err) console.log(err);
